...
Each `browse_cat_*` state corresponds to a product category (Living room, Bedroom, Dining room, Office, Outdoor). From a category state the worker can: self-loop (browse more in that category), proceed to `add_to_cart`, return to `browse_products`, or stop. `browse_products` itself also has a direct stop path. `not_found` is a dead-end 404 that loops back to `browse_products`.
