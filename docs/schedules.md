...
```bash
python generator.py -c presets/configs/ecommerce_lighting.json -w 500 --schedule presets/schedules/ecommerce.json
```

Without `--schedule`, the generator runs at full capacity (`-w` workers) at all times.

## How it works

The schedule defines a multiplier between 0 and 1 for any point in time. The generator applies this to `-w` to compute the effective worker cap:

```text
effective workers = max(1, floor(-w × schedule_multiplier))
```

At peak (multiplier = 1.0), the full `-w` is in effect. At off-peak (e.g. multiplier = 0.2), only 20% of `-w` workers are active. This means **`-w` sets the peak; the schedule shapes everything below it**.

The effective worker count is always at least 1 — even at the lowest schedule multiplier, one worker is always running.

## The ceiling

Every config has a natural concurrency ceiling determined by its state machine and interarrival time (documented in each config's README). If `-w` exceeds this ceiling, the schedule pattern will not appear cleanly at peak — the generator hits the ceiling before it reaches `-w`, producing a plateau rather than a smooth curve. Set `-w` at or below the ceiling for the schedule to drive the full shape of the data.
...
- Weights should be normalised so the maximum across the whole week is 1.0 — otherwise `-w` will never be fully utilised even at peak
